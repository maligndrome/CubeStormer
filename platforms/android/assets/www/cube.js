(typeof Cube == "object") || (Cube = {
	...
	autocompleteArray: function(arrIn){
		var current = JSON.parse(JSON.stringify(arrIn));
		var oldCube;
		do{
			oldCube = JSON.parse(JSON.stringify(current));
	...
	autocompleteEdge: function(arrIn){
		var arr = JSON.parse(JSON.stringify(arrIn));
		var completedEdges = [];
		for(var i = 0; i < 6; i++){
			for(var j = 0; j < 3; j++){
	...
	reverseArray: function(arr){
		var rarr = arr.slice(0).reverse();
		for(var i = 0; i < rarr.length; i++){
			rarr[i].direction = rarr[i].direction * -1;
		}
		return rarr;
	},
	arrayToMoveset: function(arr){
		var str = "";
	...
	solveCube: function(cube, callback, errorHandler, resultAsArray, maxDepth, maxTime, useSeparator){
		...
		var eventListener = function(event){
			if(event.data.type == "solution" && event.data.cube == cube){
				if(event.data.result.indexOf("Error") == 0 && errorHandler){
