...
function enumConst(number,string){
...
enumConst.prototype.ordinal = function(){
	return this.number;
};
function makeEnum(a){
	var o = {};
	for(var i = 0; i < a.length; i++){
		var x = new enumConst(i,a[i]);
		o[a[i]] = x;
		self[a[i]] = x;
	}
...
const Color = makeEnum(["U","R","F","D","L","B"]);
const Facelet = makeEnum(["U1", "U2", "U3", "U4", "U5", "U6", "U7", "U8", "U9", "R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8", "R9", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D9", "L1", "L2", "L3", "L4", "L5", "L6", "L7", "L8", "L9", "B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8", "B9"]);
const Corner = makeEnum(["URF","UFL","ULB","UBR","DFR","DLF","DBL","DRB"]);
const Edge = makeEnum(["UR","UF","UL","UB","DR","DF","DL","DB","FR","FL","BL","BR"]);
function verify(s){
...
const cornerFacelet = [ [ U9, R1, F3 ], [ U7, F1, L3 ], [ U1, L1, B3 ], [ U3, B1, R3 ], [ D3, F9, R7 ], [ D1, L9, F7 ], [ D7, B9, L7 ], [ D9, R9, B7 ] ];
const edgeFacelet = [ [ U6, R2 ], [ U8, F2 ], [ U4, L2 ], [ U2, B2 ], [ D6, R8 ], [ D2, F8 ], [ D4, L8 ], [ D8, B8 ], [ F6, R4 ], [ F4, L6 ], [ B6, L4 ], [ B4, R6 ] ];
const cornerColor = [ [ U, R, F ], [ U, F, L ], [ U, L, B ], [ U, B, R ], [ D, F, R ], [ D, L, F ], [ D, B, L ], [ D, R, B ] ];
const edgeColor = [ [ U, R ], [ U, F ], [ U, L ], [ U, B ], [ D, R ], [ D, F ], [ D, L ], [ D, B ], [ F, R ], [ F, L ], [ B, L ], [ B, R ] ];
function FaceCube(s){
	this.f = [];
	if(!s){
...
const N_URFtoDLB = 40320;
const N_URtoBR = 479001600;
const N_MOVE = 18;
...
function generateTables(){
	CoordCube.twistMove = generateArray(N_TWIST);
	var a = new CubieCube();
	...
	CoordCube.flipMove = generateArray(N_FLIP);
	var a = new CubieCube();
	for (i = 0; i < N_FLIP; i++) {
		a.setFlip(i);
		for (var j = 0; j < 6; j++) {
	...
	sendProgressMessage("parityMove");
	CoordCube.FRtoBR_Move = generateArray(N_FRtoBR,Int16Array,N_MOVE);
	var a = new CubieCube();
	...
	CoordCube.URFtoDLF_Move = generateArray(N_URFtoDLF,Int16Array,N_MOVE);
	var a = new CubieCube();
	...
	CoordCube.URtoDF_Move = generateArray(N_URtoDF,Int16Array,N_MOVE);
	var a = new CubieCube();
	...
	CoordCube.URtoUL_Move = generateArray(N_URtoUL,Int16Array,N_MOVE);
	var a = new CubieCube();
	...
	CoordCube.UBtoDF_Move = generateArray(N_UBtoDF,Int16Array,N_MOVE);
	var a = new CubieCube();
	...
	CoordCube.MergeURtoULandUBtoDF = generateArray(336,Int16Array,336);
	...
	CoordCube.Slice_URFtoDLF_Parity_Prun = new Int8Array((N_SLICE2 * N_URFtoDLF * N_PARITY / 2) | 0);
			...
			if (CoordCube.getPruning(CoordCube.Slice_URFtoDLF_Parity_Prun, i) == depth) {
				for (var j = 0; j < 18; j++) {
					...
					default:
						...
						if (CoordCube.getPruning(CoordCube.Slice_URFtoDLF_Parity_Prun, (N_SLICE2 * newURFtoDLF + newSlice) * 2 + newParity) == 0x0f) {
							CoordCube.setPruning(CoordCube.Slice_URFtoDLF_Parity_Prun, (N_SLICE2 * newURFtoDLF + newSlice) * 2 + newParity, (depth + 1));
							done++;
						}
					}
				}
	...
	CoordCube.Slice_URtoDF_Parity_Prun = new Int8Array((N_SLICE2 * N_URtoDF * N_PARITY / 2) | 0);
			...
			if (CoordCube.getPruning(CoordCube.Slice_URtoDF_Parity_Prun, i) == depth) {
				for (var j = 0; j < 18; j++) {
					...
					default:
						...
						var newParity = CoordCube.parityMove[parity][j];
						if (CoordCube.getPruning(CoordCube.Slice_URtoDF_Parity_Prun, (N_SLICE2 * newURtoDF + newSlice) * 2 + newParity) == 0x0f) {
							CoordCube.setPruning(CoordCube.Slice_URtoDF_Parity_Prun, (N_SLICE2 * newURtoDF + newSlice) * 2 + newParity,
								(depth + 1));
							done++;
						}
					}
	...
	CoordCube.Slice_Twist_Prun = new Int8Array((N_SLICE1 * N_TWIST / 2 + 1) | 0);
	...
	while (done != N_SLICE1 * N_TWIST) {
		for (var i = 0; i < N_SLICE1 * N_TWIST; i++) {
			...
			if (CoordCube.getPruning(CoordCube.Slice_Twist_Prun, i) == depth) {
				for (var j = 0; j < 18; j++) {
					var newSlice = (CoordCube.FRtoBR_Move[slice * 24][j] / 24) | 0;
					var newTwist = CoordCube.twistMove[twist][j];
					if (CoordCube.getPruning(CoordCube.Slice_Twist_Prun, N_SLICE1 * newTwist + newSlice) == 0x0f) {
						CoordCube.setPruning(CoordCube.Slice_Twist_Prun, N_SLICE1 * newTwist + newSlice, (depth + 1));
						done++;
					}
				}
			}
	...
	CoordCube.Slice_Flip_Prun = new Int8Array((N_SLICE1 * N_FLIP / 2) | 0);
		...
		for (var i = 0; i < N_SLICE1 * N_FLIP; i++) {
			...
			if (CoordCube.getPruning(CoordCube.Slice_Flip_Prun, i) == depth) {
				for (var j = 0; j < 18; j++) {
					...
					if (CoordCube.getPruning(CoordCube.Slice_Flip_Prun, N_SLICE1 * newFlip + newSlice) == 0x0f) {
						CoordCube.setPruning(CoordCube.Slice_Flip_Prun, N_SLICE1 * newFlip + newSlice, (depth + 1));
						done++;
					}
				}
			}
